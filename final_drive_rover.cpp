...
#define CPR_FL 200000.0
#define CPR_FR 200000.0
#define CPR_BL 200000.0
#define CPR_BR 200000.0
...
void stopDrive()
{
    ...
    {
      if(drive_speed>0)
        drive_speed = constrain(drive_speed - 5, 0, max_pwm);
      else if(drive_speed<0)
        drive_speed = constrain(drive_speed + 5, -max_pwm, 0);
      applySpeedToDrives(constrain(abs(drive_speed), 0, 255));
      delay(50);
    }
  current_dir = 0;
...
void spotTurnLeft()
{
    while(drive_speed<30)
    {
      drive_speed = constrain(drive_speed + 5, -30, 30);
      applySpeedToDrives(constrain(drive_speed, 0, 255));
      delay(5);
    }
  mFL.backward();
...
rcl_subscription_t en_reset;
std_msgs__msg__Int16 encoder_reset_msg;


rcl_publisher_t debug;
rcl_publisher_t odom_pub; //publishing to /rover_odom
std_msgs__msg__String odom_msg;
...
void encoder_reset(const void *msgin)
{
  ...
  if (value == 1)
  {
    FrontLeft.setCount(0);
    FrontRight.setCount(0);
    BackLeft.setCount(0);
    BackRight.setCount(0);
    publish_debug("Encoders & odometry reset");
  }
}
...
void rover_cmd(const void *msgin)
{
  ...
  case 1:
    ...
    max_pwm = 50;
    setDriveDirection(+1);
    current_dir = 1;        //set +1 so that applyspeedtodrives knows dir

    while(drive_speed<max_pwm) //increase speed gradually
    {
      drive_speed = constrain(drive_speed + 5, -max_pwm, max_pwm);
      applySpeedToDrives(constrain(drive_speed, 0, 255));
      delay(5);
    }
    driveForward(); //forward at const pwm
    break;
  ...
  case 2:
    ...
    max_pwm = 50;
    setDriveDirection(-1);  //set -1 so that applyspeedtodrives knows dir
    current_dir = -1; 

    while(drive_speed>-max_pwm)  //decrease speed gradually
    {
      drive_speed = constrain(drive_speed - 5, -max_pwm, max_pwm);
      applySpeedToDrives(constrain(drive_speed, 0, 255));
      delay(5);
    }
    driveBackward(); //backward at const pwm
    break;
...
void setup()
{
  ...
  RCCHECK(rclc_publisher_init_default(
    ...
    odom_msg.data.data = odom_buf;
    odom_msg.data.capacity = sizeof(odom_buf);
  ...
  RCCHECK(rclc_executor_add_subscription(&executor, &en_reset, &msg, &encoder_reset, ON_NEW_DATA));
...
void loop()
{
  ...
  snprintf(odom_buf, sizeof(odom_buf),
           "%lld %lld %lld %lld",
  ...
  odom_msg.data.size = strlen(odom_buf);
  rcl_publish(&odom_pub, &odom_msg, NULL);
